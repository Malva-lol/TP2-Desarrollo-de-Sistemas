...
authRouter.post("/register", async (req, res) => {
	try {
		...
		const password_hash = contrasena; // it doesn't hash at all
		const user = await userService1.createUsuario(nombre, telefono, email, password_hash, true, direccion, 0, "");
...
authRouter.post("/login", async (req, res) => {
	try {
		...
		const session_token = await userVerificationService1.generateUserSession(user.id);
